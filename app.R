...
ui <- fluidPage(
  fluidRow(
    ...
    column(
      ...
      sliderInput(
        inputId = "gradient",
        label = "gradient max for the map",
        min = .15,
        max = 1,
        value = .6
      )
...
server <- function(input, output, session) {
  output$summary <- renderGirafe({
    ...
    gg <- ggplot(
      dat,
      aes(x = x, y = score, fill = nuance, tooltip = tooltip, data_id = nuance)
    ) +
      ...
      theme_void(base_family = "Liberation Sans") +
      theme(
        plot.caption = element_text_interactive(
  ...
  output$map <- renderGirafe({
    ...
    # add tooltips ----
    ft_tooltips <- flextables_tour_secteur[
      flextables_tour_secteur$TOUR %in% tour(),
    ]
    formula_highlight <- as.formula(sprintf("~ nuance %%in%% '%s'", nuance()))
    ft_tooltips$ft <- lapply(
      ft_tooltips$ft,
      function(ft, f) {
    ...
    gmap <- CarteSimple +
      ...
      scale_fill_gradientn(
        colors = rev(c("#000004FF", "#BB3754FF", "#FCFFA4FF")),
        limits = c(0, input$gradient),
        na.value = "#000004FF"
      ) +
